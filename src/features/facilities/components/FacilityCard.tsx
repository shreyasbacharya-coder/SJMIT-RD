...
export function FacilityCard({ facility, isAdmin, onEdit, onDelete, defaultOpen = !isAdmin }: FacilityCardProps) {
  ...
  const handleToggleAvailability = (equipmentIndex: number, newIsAvailable: boolean) => {
    if (!facility.equipments || !isAdmin) return;

    const updatedEquipments = [...facility.equipments];
    ...
    updateFacility.mutate({ id: facility.id, data: { equipments: updatedEquipments } });
  };

  const hasEquipments = facility.equipments && facility.equipments.length > 0;

  return (
      <Card className="p-6 border-l-4 border-l-brand-start bg-slate-50 dark:bg-white/5 relative overflow-hidden group">
        <div className="absolute top-0 right-0 w-64 h-64 bg-brand-start/5 blur-[100px] -z-10 group-hover:bg-brand-start/10 transition-colors duration-700" />
        <div className="flex flex-col md:flex-row justify-between gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-4 mb-2">
              <h3 className="text-xl font-display font-bold text-slate-900 dark:text-white tracking-tight">{facility.labName}</h3>
              <span className="px-4 py-1 bg-brand-start/10 text-brand-start text-[10px] font-bold rounded-full uppercase tracking-widest border border-brand-start/20">
                {facility.equipments?.length || 0} Equipments
              </span>
            </div>
            
            <p className="text-slate-500 dark:text-slate-400 font-light mb-4 flex items-center gap-3 text-base">
              <MapPin size={16} className="text-brand-start" />
              {facility.department}
            </p>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {facility.facultyInCharge && (
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] block">Faculty In-Charge</span>
                    <p className="text-base font-bold text-slate-900 dark:text-white">{facility.facultyInCharge}</p>
                  </div>
                )}
                {facility.workingDays && (
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] block">Availability</span>
                    <p className="text-base font-bold text-slate-900 dark:text-white">{facility.workingDays}</p>
                  </div>
                )}
              </div>
              ...
              {facility.researchFocus && (
                <div className="space-y-2">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] block">Research Focus</span>
                  <p className="text-slate-500 dark:text-slate-400 leading-relaxed font-light">{facility.researchFocus}</p>
                </div>
              )}

              <div className="flex flex-wrap gap-4 pt-3 border-t border-slate-200 dark:border-white/5">
                {facility.email && (
                  <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400 hover:text-brand-start transition-colors">
                    <Mail size={16} className="text-brand-start" />
                    <span className="text-sm font-light">{facility.email}</span>
                  </div>
                )}
                {facility.contact && (
                  <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400 hover:text-brand-start transition-colors">
                    <Phone size={16} className="text-brand-start" />
                    <span className="text-sm font-light">{facility.contact}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        {isAdmin && hasEquipments && (
          <button
            ...
            className="w-full flex items-center justify-center gap-3 mt-6 pt-6 border-t border-slate-200 dark:border-white/5 cursor-pointer text-brand-start font-bold text-sm"
          >
          ...
          hasEquipments ? (
            <div className="space-y-3 pt-6">
              <div className="flex items-center gap-4">
                ...
                <span className="text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-[0.3em]">Facility Inventory</span>
                <div className="h-px flex-1 bg-slate-200 dark:bg-white/5" />
              </div>
            ...
            <div className="py-12 text-center bg-slate-50 dark:bg-white/5 rounded-3xl border border-dashed border-slate-200 dark:border-white/10 mt-8">
              <p className="text-slate-400 dark:text-slate-500 font-light italic">No equipment listed for this facility.</p>
            </div>
          )
        )}
      </Card>
