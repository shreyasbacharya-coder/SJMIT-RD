...
function AdminDepartmentDetailContent({ id }: { id: string }) {
  const { data: viewingDepartment, isLoading } = useDepartment(id);
  ...
  return (
    <PageContainer>
      ...
      <div className="space-y-6 bg-white dark:bg-white/5 p-8 rounded-3xl">
        {viewingDepartment.image && (
          <img
            src={viewingDepartment.image}
            alt={viewingDepartment.name}
        ...
        <div className="flex justify-end pt-6 border-t border-slate-200 dark:border-white/10">
          <Link href={`/departments?id=${viewingDepartment.id}`} target="_blank" rel="noopener noreferrer" className="btn-secondary py-2 px-6 text-sm group">
...
export function AdminDepartmentDetailClientPage({ id }: { id: string }) {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-brand-start" size={48} /></div>}>
      <AdminDepartmentDetailContent id={id} />
    </Suspense>
  );
}
